...
static int _php_count_values(zval *array)
{
	...
	for (zend_hash_internal_pointer_reset(Z_ARRVAL_P(array));
		 zend_hash_get_current_data(Z_ARRVAL_P(array)) != NULL;
		 zend_hash_move_forward(Z_ARRVAL_P(array))) {
		
		pzval = zend_hash_get_current_data(Z_ARRVAL_P(array));
		if (Z_TYPE_P(pzval) == IS_ARRAY) {
			values += zend_hash_num_elements(Z_ARRVAL_P(pzval));
		}
	}
	return values;
}
...
static struct svm_problem* php_svm_read_array(php_svm_object *intern, php_svm_model_object *intern_model, zval *array TSRMLS_DC)
{
	...
	for (zend_hash_internal_pointer_reset(Z_ARRVAL_P(array)), i = 0;
		...
		ppzval = zend_hash_get_current_data(Z_ARRVAL_P(array));
		...
		if (zend_hash_num_elements(Z_ARRVAL_P(ppzval)) < 2) {
		...
		zend_hash_internal_pointer_reset(Z_ARRVAL_P(ppzval));
		
		if ((zend_hash_get_current_data(Z_ARRVAL_P(ppzval)) != NULL)) {
		...
		while (1) {
			...
			if ((zend_hash_move_forward(Z_ARRVAL_P(ppzval)) == SUCCESS) && 
				zend_hash_get_current_data(Z_ARRVAL_P(ppzval)) != NULL) {	
				
				ppz_value = zend_hash_get_current_data(Z_ARRVAL_P(ppzval));		

				if (zend_hash_get_current_key(Z_ARRVAL_P(ppzval), &key, &index) == HASH_KEY_IS_STRING) {
					intern_model->x_space[j].index = (int) strtol(key->val, &endptr, 10);
				} else {
...
static struct svm_node* php_svm_get_data_from_array(zval* arr TSRMLS_DC) 
{
	...
	for(zend_hash_internal_pointer_reset_ex(arr_hash, &pointer);
		zend_hash_get_current_data_ex(arr_hash, &pointer) != NULL; 
		zend_hash_move_forward_ex(arr_hash, &pointer)) 
	{
		if (zend_hash_get_current_key_ex(arr_hash, &key, &index, &pointer) == HASH_KEY_IS_STRING) {
			x[i].index = (int) strtol(key->val, &endptr, 10);
		} else {
			x[i].index = (int) index;
		} 
		temp = **data;
...
PHP_METHOD(svm, __construct)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS() TSRMLS_CC, getThis(), "O", &intern) == FAILURE) {
...
PHP_METHOD(svm, setOptions) 
{
	...
	for (zend_hash_internal_pointer_reset(params_ht);
		 zend_hash_get_current_data(params_ht) != NULL;
		 zend_hash_move_forward(params_ht)) {
		
		ppzval = zend_hash_get_current_data(params_ht);
		...
		/* Bool attribute */
			...
			if (Z_TYPE_P(tmp_pzval) != IS_FALSE && Z_TYPE_P(tmp_pzval) != IS_TRUE) {
...
PHP_METHOD(svm, crossvalidate)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS() TSRMLS_CC, tempobj, "O", &intern_return) == FAILURE) {
        return;
	}
...
PHP_METHOD(svm, train)
{
	...
	if(weights) {
		...
		if(zend_hash_num_elements(weights_ht) > 0) {
			...
			for (zend_hash_internal_pointer_reset(weights_ht), i = 0;
				 zend_hash_get_current_data(weights_ht) != NULL;
				 zend_hash_move_forward(weights_ht), i++) {

				zval tmp_zval, *tmp_pzval;
				
				ppzval = zend_hash_get_current_data(weights_ht);
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS() TSRMLS_CC, return_value, "O", &intern_return) == FAILURE) {
        return;
	}
